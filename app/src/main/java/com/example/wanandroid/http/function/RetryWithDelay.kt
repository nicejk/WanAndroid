package com.example.wanandroid.http.function
 ...
 * @Description：请求重连
 * @Author: haishan
 * @CreateDate: 2020/10/15 11:35 AM
 */
class RetryWithDelay : Function<Observable<out Throwable>, Observable<*>> {
    ...
    override fun apply(observable: Observable<out Throwable>): Observable<*> {
        return observable
                .zipWith(Observable.range(1, maxRetryCount + 1), BiFunction<Throwable, Int, Wrapper> { t1, t2 -> Wrapper(t2, t1) })
                .flatMap { wrapper ->
                    val t = wrapper.throwable
                    if ((t is ConnectException
                                    || t is SocketTimeoutException
                                    || t is TimeoutException
                                    || t is HttpException) && wrapper.index < maxRetryCount + 1) {
